class OrdersController < Spree::BaseController     
  ...
  private
  def build_object
    logger.info "BUILD OBJECT IS FIRED"
    @object ||= find_order
  end
  
  def object
    logger.info "OBJECT IS FIRED"
    logger.info "@object is nil? #{@object.nil?.to_s}"
    logger.info "@order is nil? #{@order.nil?.to_s}"
    logger.info "Is order valid? #{@object.valid?.to_s}"
    logger.info "ERRORS: #{@object.errors.full_messages}"
    @object ||= Order.find_by_number(params[:id], :include => :adjustments) if params[:id]
    return @object || find_order
  end
  
  def create_before
    logger.info "CREATE BEFORE IS FIRED"
